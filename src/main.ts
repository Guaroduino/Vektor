...
class VektorApp {
  ...
  private async setup() {
    ...
    await this.app.init({
      ...
      // Using renderer.extract for exports; preserveDrawingBuffer is not required
  ...
  private async exportPNG() {
    try {
      ...
      // Snapshot the on-screen canvas and encode as PNG (no ExtractSystem)
  ...
  private async exportJPG() {
    try {
      // Mirror the PNG snapshot path, but composite onto the app background color before JPEG encoding
      ...
      if (sourceCanvas && (sourceCanvas as any).width && (sourceCanvas as any).height) {
        ...
        const ctx = out.getContext('2d');
        if (ctx) {
          // Pre-fill with current background so JPEG (no alpha) matches on-screen appearance
